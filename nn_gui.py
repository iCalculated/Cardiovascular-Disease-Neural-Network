...
names = ["Age (years)", "Sex (1 = male, 0 = female)",
         ...
         "Resting electrocardiographic testing results (0 = normal, 1 = having ST-T wave abnormality, 2 = showing probable or definite left ventricular hypertrophy)",
...
def createNN(x, y):
    ...
    # if the score is an error value, redo the trials.
    if scores[1] > 0.5385 and scores[1] < 0.5390:
        print("    " + "Error occurred. Rerunning trials.")
        return 1
    else:
        score = (scores[1] * 100)
        return 0
...
def trials(X, Y):
    # this loop facilitates the rerunning of trialss
    succ = createNN(X, Y)
    if succ == 1:
        trials(X, Y)
...
nameList = Label(root,
              ...
              School''',
                 fg="#383a39", bg="#a1dbcd", font=("Helvetica", 24))
nameList.pack()
